...
RUN_NAMES = tuple(f"pilot_fd_eps32_alpha8_fw{weight}" for weight in (50, 100, 200, 400))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Summarize high-FD epsilon=32 pilots")
    parser.add_argument(
        "output_root",
        nargs="?",
        default="/data/cjk/FGSM-MEP-cifar10-eps32-alpha8-highfd",
    )
    ...
    print(
        "| Run | FD weight | Best epoch | Best clean | Best PGD-10 | Final clean "
        "| Final PGD-10 | CE | 10×logit | λ×feature |"
    )
    print("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for item in results:
        print(
            f"| {item['name']} | {item['weight']:g} | {item['epoch']} "
            f"| {item['best_clean']:.2f} | {item['best_pgd']:.2f} "
